...
document.getElementById('excelFile').addEventListener('change', async (e) => {
    ...
    try {
        const response = await fetch('/upload-excel-stickers', {
            method: 'POST',
            body: formData
        });
...
document.getElementById('pptFile').addEventListener('change', async (e) => {
    ...
    try {
        const response = await fetch('/upload-powerpoint-stickers', {
            method: 'POST',
            body: formData
        });
        const data = await response.json();
...
document.getElementById('transferBtn').addEventListener('click', async () => {
    showProgress(true);
    try {
        const response = await fetch('/transfer-stickers', {
            method: 'POST'
        });
        ...
        if (response.ok) {
            ...
            a.href = url;
            a.download = 'modified_presentation_stickers.pptx';
